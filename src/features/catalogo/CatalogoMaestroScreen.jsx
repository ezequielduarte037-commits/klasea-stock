...
function Metric({ label, value, color = C.text, hint = "" }) {
  return (
    <div style={{ minWidth: 0, padding: "9px 10px", border: `1px solid ${C.border}`, background: C.panel, borderRadius: 10 }}>
      <div style={{ color: C.dim, fontSize: 9, fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase" }}>{label}</div>
      <div style={{ color, fontFamily: C.mono, fontSize: 16, fontWeight: 750, marginTop: 3 }}>{value}</div>
      ...
      <div role="dialog" aria-modal="true" style={{ width: "min(560px, 100%)", border: `1px solid ${C.border}`, borderRadius: 16, background: C.panelSolid, boxShadow: "0 28px 90px rgba(0,0,0,.42)", overflow: "hidden" }}>
        <div style={{ display: "flex", gap: 11, alignItems: "flex-start", padding: 16, borderBottom: `1px solid ${C.border}` }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ color: C.text, fontSize: 16, fontWeight: 750 }}>Confirmar cambio de identidad</div>
        ...
        <div style={{ padding: 16, display: "grid", gap: 12 }}>
          <div style={{ color: C.text, fontSize: 13, fontWeight: 700 }}>{material?.descripcion}</div>
          ...
          <div style={{ border: `1px solid ${C.border}`, background: C.panel, borderRadius: 11, padding: 11, display: "grid", gap: 7 }}>
            <div style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>Campos que cambian</div>
            {changes.map((change) => <div key={change.label} style={{ display: "grid", gridTemplateColumns: "100px minmax(0,1fr) 14px minmax(0,1fr)", gap: 7, alignItems: "center", color: C.text, fontSize: 11.5 }}><b>{change.label}</b><span style={{ color: C.dim, overflow: "hidden", textOverflow: "ellipsis" }}>{change.before || "—"}</span><ChevronRight size={13} color={C.blue} /><span style={{ color: C.blue, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis" }}>{change.after || "—"}</span></div>)}
          </div>
        </div>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: "12px 16px", borderTop: `1px solid ${C.border}` }}>
          <button type="button" onClick={onCancel} disabled={busy} style={{ border: `1px solid ${C.border}`, background: C.panel, color: C.text, borderRadius: 9, padding: "8px 12px", cursor: "pointer", fontWeight: 700 }}>Volver</button>
          <button type="button" onClick={onConfirm} disabled={busy} style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${C.violetB}`, background: C.violet, color: "#fff", borderRadius: 9, padding: "8px 13px", cursor: busy ? "default" : "pointer", opacity: busy ? 0.65 : 1, fontWeight: 700 }}><Check size={14} />{busy ? "Guardando…" : "Confirmar cambio"}</button>
        </div>
      </div>
    </div>
  );
...
export default function CatalogoMaestroScreen({ profile }) {
  ...
  const detail = selected && (
    <section style={{ minWidth: 0, minHeight: 0, border: `1px solid ${C.border}`, background: C.panelSolid, borderRadius: 13, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "13px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ color: C.text, fontSize: 16, fontWeight: 750, lineHeight: 1.25 }}>{selected.descripcion}</div>
        ...
        {canEdit && !editing && <button type="button" onClick={() => setEditing(true)} style={{ display: "inline-flex", alignItems: "center", gap: 5, border: `1px solid ${C.blueB}`, background: C.blueL, color: C.blue, borderRadius: 8, padding: "7px 9px", cursor: "pointer", fontSize: 11.5, fontWeight: 700 }}><Edit3 size={13} />Editar</button>}
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: 12, display: "grid", gap: 12, alignContent: "start" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <button type="button" onClick={() => nav(`/stock-panol?tab=maestro&material=${selected.id}&q=${encodeURIComponent(selected.descripcion)}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${C.greenB}`, background: C.greenL, color: C.green, borderRadius: 9, padding: "7px 10px", cursor: "pointer", fontSize: 11.5, fontWeight: 700 }}><Warehouse size={14} />Ver en pañol <ArrowRight size={13} /></button>
        </div>
        ...
        {editing ? (
          <div style={{ border: `1px solid ${C.blueB}`, background: C.blueL, borderRadius: 12, padding: 12, display: "grid", gap: 10 }}>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(0,1.8fr) minmax(130px,.7fr)", gap: 9 }}>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Nombre *</span><input value={draft.descripcion} onChange={(event) => setDraft((current) => ({ ...current, descripcion: event.target.value }))} style={inputStyle} /></label>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Unidad</span><input value={draft.unidad_medida} onChange={(event) => setDraft((current) => ({ ...current, unidad_medida: event.target.value }))} style={inputStyle} /></label>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 9 }}>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Código interno</span><input value={draft.codigo} onChange={(event) => setDraft((current) => ({ ...current, codigo: event.target.value }))} style={inputStyle} /></label>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Código de barras</span><input value={draft.codigo_barra} onChange={(event) => setDraft((current) => ({ ...current, codigo_barra: event.target.value }))} style={inputStyle} /></label>
            </div>
            <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Alias de búsqueda</span><input value={draft.alias} onChange={(event) => setDraft((current) => ({ ...current, alias: event.target.value }))} placeholder="Sinónimos separados por coma" style={inputStyle} /></label>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 9 }}>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Rubro</span><select value={draft.categoria_id} onChange={(event) => setDraft((current) => ({ ...current, categoria_id: event.target.value }))} style={inputStyle}><option value="">Sin rubro</option>{data.categorias.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label>
              <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Proveedor</span><input list="catalogo-proveedores" value={draft.proveedor} onChange={(event) => setDraft((current) => ({ ...current, proveedor: event.target.value }))} style={inputStyle} /><datalist id="catalogo-proveedores">{data.proveedores.filter((item) => item.activo !== false).map((item) => <option key={item.id} value={item.nombre} />)}</datalist></label>
            </div>
            <label style={{ display: "grid", gap: 5, maxWidth: isMobile ? "none" : 220 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Stock mínimo</span><input inputMode="decimal" value={draft.stock_minimo} onChange={(event) => setDraft((current) => ({ ...current, stock_minimo: event.target.value }))} placeholder="Sin mínimo" style={inputStyle} /><span style={{ color: C.dim, fontSize: 9.5 }}>Define la alerta de reposición; no modifica el saldo.</span></label>
            <label style={{ display: "grid", gap: 5 }}><span style={{ color: C.dim, fontSize: 9.5, fontWeight: 700, textTransform: "uppercase" }}>Notas y palabras de búsqueda</span><textarea rows={3} value={draft.notas} onChange={(event) => setDraft((current) => ({ ...current, notas: event.target.value }))} style={{ ...inputStyle, resize: "vertical" }} /></label>
            <label style={{ display: "flex", alignItems: "center", gap: 8, color: C.text, fontSize: 12, fontWeight: 700 }}><input type="checkbox" checked={draft.activo} onChange={(event) => setDraft((current) => ({ ...current, activo: event.target.checked }))} />Producto activo</label>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button type="button" onClick={() => { setDraft(emptyDraft(selected)); setEditing(false); }} disabled={saving} style={{ border: `1px solid ${C.border}`, background: C.panel, color: C.text, borderRadius: 9, padding: "8px 11px", cursor: "pointer", fontWeight: 700 }}>Cancelar</button>
              <button type="button" onClick={requestSave} disabled={!dirty || saving} style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${dirty ? C.greenB : C.border}`, background: dirty ? C.green : C.panel2, color: dirty ? "#fff" : C.dim, borderRadius: 9, padding: "8px 12px", cursor: dirty && !saving ? "pointer" : "default", fontWeight: 700 }}><Save size={14} />{saving ? "Guardando…" : "Guardar ficha"}</button>
            </div>
          </div>
        ) : (
          <div style={{ border: `1px solid ${C.border}`, background: C.panel, borderRadius: 12, padding: 12, display: "grid", gap: 9 }}>
            ...
            ].map(([label, value]) => <div key={label} style={{ display: "grid", gridTemplateColumns: "120px minmax(0,1fr)", gap: 9, fontSize: 11.5 }}><span style={{ color: C.dim, fontWeight: 700 }}>{label}</span><span style={{ color: C.text, lineHeight: 1.4 }}>{value}</span></div>)}
          </div>
        )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr)", height: "100%" }}>
        <main style={{ minWidth: 0, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <header style={{ minHeight: 52, display: "flex", alignItems: "center", gap: 10, padding: isMobile ? "9px 12px" : "9px 18px", borderBottom: `1px solid ${C.border}`, background: C.topbar, flexShrink: 0 }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}><div style={{ color: C.text, fontSize: 17, fontWeight: 750 }}>Catálogo maestro</div><div style={{ color: C.dim, fontSize: 10.5, marginTop: 1 }}>Identidad de productos · sin editar cantidades de stock</div></div>
          ...
          <div style={{ padding: isMobile ? 10 : "12px 16px", borderBottom: `1px solid ${C.border}`, background: C.topbarSoft, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", flexShrink: 0 }}>
            ...
            <span style={{ color: C.dim, fontFamily: C.mono, fontSize: 11.5, fontWeight: 700 }}>{filtered.length} productos</span>
          </div>
          ...
          <div style={{ flex: 1, minHeight: 0, padding: isMobile ? 10 : 14, display: "grid", gridTemplateColumns: isMobile || !selectedVisible ? "minmax(0,1fr)" : "minmax(420px,1.15fr) minmax(360px,.85fr)", gap: 12, overflow: "hidden" }}>
            {!(isMobile && selectedVisible) && <section style={{ minWidth: 0, minHeight: 0, border: `1px solid ${C.border}`, background: C.panel, borderRadius: 13, overflow: "hidden", display: "flex", flexDirection: "column" }}>
              <div style={{ padding: "8px 12px", borderBottom: `1px solid ${C.border}`, background: C.panelSolid, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}><div style={{ display: "flex", alignItems: "center", gap: 7, color: C.text, fontSize: 12.5, fontWeight: 700 }}><Boxes size={14} color={C.blue} />Productos</div><span style={{ color: C.dim, fontSize: 10.5 }}>La existencia se consulta, no se edita acá</span></div>
              <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: 7, display: "grid", gap: 5, alignContent: "start" }}>
                {loading ? <div style={{ padding: 32, color: C.dim, textAlign: "center", fontSize: 12.5 }}>Cargando catálogo…</div> : filtered.length ? <>
                  {filtered.slice(0, limit).map((material) => {
                    ...
                    return <button key={material.id} type="button" onClick={() => selectMaterial(material)} style={{ width: "100%", display: "grid", gridTemplateColumns: "38px minmax(0,1fr) auto", alignItems: "center", gap: 9, border: `1px solid ${active ? C.blueB : C.border}`, background: active ? C.blueL : C.panelSolid, borderRadius: 10, padding: "8px 9px", color: C.text, textAlign: "left", cursor: "pointer", fontFamily: C.sans }}><MaterialThumb material={material} size={36} /><span style={{ minWidth: 0 }}><span style={{ display: "block", fontSize: 12.5, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{material.descripcion}</span><span style={{ display: "block", color: C.dim, fontSize: 10.5, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{material.codigo || "sin código"}{material.alias ? ` · ${material.alias}` : ""} · {categoryById.get(material.categoria_id) || "Sin rubro"}</span></span><span style={{ display: "inline-flex", alignItems: "center", gap: 4, color: material.activo === false ? C.dim : C.green, border: `1px solid ${material.activo === false ? C.border : C.greenB}`, background: material.activo === false ? C.panel2 : C.greenL, borderRadius: 999, padding: "2px 7px", fontSize: 9.5, fontWeight: 700 }}>{material.activo === false ? "Archivado" : "Activo"}<ChevronRight size={11} /></span></button>;
                  })}
                  {limit < filtered.length && <button type="button" onClick={() => setLimit((current) => current + PAGE_SIZE)} style={{ border: `1px solid ${C.blueB}`, background: C.blueL, color: C.blue, borderRadius: 9, padding: 9, cursor: "pointer", fontWeight: 700 }}>Mostrar {Math.min(PAGE_SIZE, filtered.length - limit)} más</button>}
                </> : <div style={{ padding: 32, textAlign: "center", color: C.dim, fontSize: 12.5 }}>No hay productos para esos filtros.</div>}
              </div>
            </section>}
            {selectedVisible ? detail : !isMobile && <section style={{ minHeight: 0, border: `1px dashed ${C.border}`, borderRadius: 13, display: "grid", placeItems: "center", padding: 24 }}><div style={{ textAlign: "center", maxWidth: 310 }}><PackageSearch size={34} color={C.blue} /><div style={{ color: C.text, fontSize: 16, fontWeight: 750, marginTop: 9 }}>Elegí un producto</div><div style={{ color: C.dim, fontSize: 12, lineHeight: 1.45, marginTop: 5 }}>Vas a ver su identidad, el impacto y el saldo vinculado sin mezclarlo con el stock físico.</div></div></section>}
          </div>
        </main>
      </div>
