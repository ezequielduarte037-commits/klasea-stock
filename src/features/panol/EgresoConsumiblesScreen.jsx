...
export default function EgresoConsumiblesScreen({ profile }) {
          ...
          <div style={{ flex: 1, minHeight: 0, overflow: "auto", padding: isMobile ? 12 : 16, display: "grid", gap: 14, gridTemplateColumns: isMobile ? "1fr" : "minmax(0,1.35fr) minmax(0,1fr)", alignContent: "start" }}>
            ...
            <div style={{ display: "grid", gap: 14, alignContent: "start" }}>
              <section style={{ ...panel, padding: 16, borderColor: tono }}>
                ...
                <p style={{ margin: "9px 0 0", color: C.dim, fontSize: 11.5, fontWeight: 650 }}>
              ...
              <section style={{ ...panel, overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "12px 15px", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontSize: 14, fontWeight: 750, color: C.text }}>{esEgreso ? "Se lleva" : "Entra"}</span>
                  <span style={{ fontFamily: C.mono, fontSize: 13, fontWeight: 700, color: carrito.length ? tono : C.dim }}>{totalItems || "—"}</span>
                  {carrito.length ? (
                    <button type="button" onClick={() => { setCarrito([]); enfocarEscaneo(); }} className="caja-btn"
                      style={{ marginLeft: "auto", border: `1px solid ${C.border2}`, background: "transparent", color: C.dim, borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontWeight: 650, fontFamily: C.sans }}>Vaciar</button>
                  ) : null}
                </div>

                {!carrito.length ? (
                  <div style={{ padding: "38px 16px", textAlign: "center", color: C.dim, fontSize: 13, fontWeight: 650 }}>
                      ...
                      return (
                        <div key={x.id} className={`caja-item${ultimo?.id === x.id ? " caja-nuevo" : ""}`}
                          ...
                          <div style={{ minWidth: 0, flex: 1 }}>
                            <div style={{ color: C.text, fontSize: 13.5, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{x.descripcion}</div>
                            <div style={{ color: esEgreso && quedan < 0 ? C.red : C.dim, fontSize: 11, fontWeight: 650 }}>
                              {x.unidad}
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
                            ...
                            <span style={{ fontFamily: C.mono, fontSize: 15, fontWeight: 750, minWidth: 36, textAlign: "center", fontVariantNumeric: "tabular-nums" }}>{fmt(x.cantidad)}</span>
                    ...
                    <div style={{ padding: 14 }}>
                      <button type="button"
                        ...
                        style={{ width: "100%", border: "none", borderRadius: 11, padding: "16px 18px", cursor: guardando ? "default" : "pointer",
                          background: tono, color: "#fff", fontFamily: C.sans, fontSize: 15.5, fontWeight: 750,
            ...
            <section style={{ ...panel, overflow: "hidden", alignSelf: "start" }}>
              <div style={{ padding: "12px 15px", borderBottom: `1px solid ${C.border}`, fontSize: 13.5, fontWeight: 700, color: C.text }}>Lo último que pasó</div>
              {!historial.length ? (
                <div style={{ padding: "28px 16px", textAlign: "center", color: C.dim, fontSize: 12.5, fontWeight: 650 }}>Todavía no hay movimientos.</div>
              ) : historial.map((h) => {
                  ...
                  <div key={h.id} style={{ padding: "9px 15px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 9, alignItems: "flex-start" }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                        <span style={{ fontFamily: C.mono, fontSize: 12.5, fontWeight: 700, color: C.text }}>{fmt(h.cantidad_egresada || h.cantidad)}</span>
                        <span style={{ color: C.text, fontSize: 12.5, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{h.descripcion}</span>
                      </div>
          ...
          <div ref={firmaRef} tabIndex={-1}
            ...
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12 }}>
              <button type="button" onClick={volverAMarcar} className="caja-btn"
                style={{ border: `1px solid ${C.border2}`, background: "var(--panel-2)", color: C.text, borderRadius: 9, padding: "8px 12px", cursor: "pointer", fontFamily: C.sans, fontSize: 12.5, fontWeight: 650, display: "inline-flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                <ArrowLeft size={14} /> Seguir marcando
              </button>
              <div style={{ minWidth: 0, flex: 1, textAlign: "right" }}>
                <div style={{ color: C.text, fontSize: 16.5, fontWeight: 750 }}>Se lleva {totalItems} {totalItems === 1 ? "cosa" : "cosas"}</div>
                <div style={{ color: C.dim, fontSize: 11.5, fontWeight: 650 }}>{carrito.length === 1 ? "un solo producto" : `${carrito.length} productos distintos`}</div>
              </div>
            </div>
            ...
            <div style={{ flex: "0 1 auto", overflowY: "auto", borderBottom: `1px solid ${C.border}`, maxHeight: 230 }}>
              {carrito.map((x) => (
                <div key={x.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 20px", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontFamily: C.mono, fontSize: 15, fontWeight: 750, color: tono, minWidth: 34, fontVariantNumeric: "tabular-nums" }}>{fmt(x.cantidad)}</span>
                  <span style={{ color: C.text, fontSize: 13.5, fontWeight: 650, minWidth: 0, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{x.descripcion}</span>
                  <span style={{ color: C.dim, fontSize: 11.5, fontWeight: 600, flexShrink: 0 }}>{x.unidad}</span>
                </div>
              ))}
              {sector || obraId ? (
                <div style={{ padding: "9px 20px", color: C.dim, fontSize: 12, fontWeight: 650 }}>
            ...
            <div style={{ padding: 20, display: "grid", gap: 15 }}>
              {persona ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12, border: `1px solid ${C.greenB}`, background: "var(--green-soft)" }}>
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ color: C.text, fontSize: 15.5, fontWeight: 750, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{persona.nombre}</div>
                    <div style={{ color: C.green, fontSize: 11.5, fontWeight: 650 }}>tarjeta {persona.uid}{persona.dni ? ` · DNI ${persona.dni}` : ""}</div>
                  </div>
                  <button type="button" onClick={() => setPersona(null)} className="caja-btn"
                    style={{ border: `1px solid ${C.border2}`, background: "var(--panel-solid)", color: C.dim, borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontSize: 12, fontWeight: 650, fontFamily: C.sans }}>No soy yo</button>
                </div>
              ) : (
                <>
                  ...
                  {aMano ? (
                    <div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 7 }}>
                        <span style={{ color: limpio(nombreLibre) && !tieneApellido(nombreLibre) ? C.red : C.dim, fontSize: 11.5, fontWeight: 650 }}>
                        ...
                        <button type="button" onClick={() => { setAMano(false); setNombreLibre(""); firmaRef.current?.focus(); }} className="caja-btn"
                          style={{ marginLeft: "auto", border: "none", background: "transparent", color: C.blue, cursor: "pointer", fontFamily: C.sans, fontSize: 12, fontWeight: 700, padding: 0 }}>
                    ...
                    <button type="button" onClick={() => { setAMano(true); setTimeout(() => nombreRef.current?.focus(), 40); }} className="caja-btn"
                      style={{ border: `1px dashed ${C.border2}`, background: "transparent", color: C.dim, borderRadius: 11, padding: "11px 14px", cursor: "pointer", fontFamily: C.sans, fontSize: 13, fontWeight: 650 }}>
              ...
              <button type="button" onClick={firmar} disabled={guardando || !puedeFirmar}
                style={{ width: "100%", border: "none", borderRadius: 12, padding: "17px 18px", cursor: guardando || !puedeFirmar ? "default" : "pointer",
                  ...
                  fontFamily: C.sans, fontSize: 16, fontWeight: 750, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 9 }}>
      ...
      {paso === "listo" && comprobante ? (
        <div role="dialog" aria-modal="true" aria-label="Retiro registrado"
          ...
          <div style={{ ...panel, width: "100%", maxWidth: 520, overflow: "hidden", textAlign: "center", padding: "30px 24px 24px" }}>
            ...
            <div style={{ color: C.text, fontSize: 20, fontWeight: 750 }}>Listo</div>
            <div style={{ color: C.dim, fontSize: 13.5, fontWeight: 650, marginTop: 4 }}>
              {comprobante.total} {comprobante.total === 1 ? "cosa" : "cosas"} a nombre de <strong style={{ color: C.text, fontWeight: 700 }}>{comprobante.quien}</strong>
            </div>
            <div style={{ color: C.dim, fontSize: 11.5, fontWeight: 600, marginTop: 3 }}>
            ...
            <div style={{ margin: "18px 0 20px", textAlign: "left", border: `1px solid ${C.border}`, borderRadius: 11, overflow: "hidden", maxHeight: 190, overflowY: "auto" }}>
              {comprobante.items.map((x) => (
                <div key={x.id} style={{ display: "flex", gap: 9, padding: "8px 13px", borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontFamily: C.mono, fontSize: 13, fontWeight: 700, color: C.text, minWidth: 30 }}>{fmt(x.cantidad)}</span>
                  <span style={{ color: C.text, fontSize: 12.5, fontWeight: 650, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{x.descripcion}</span>
                </div>
              ))}
            </div>
            <button type="button" onClick={empezarDeNuevo}
              style={{ width: "100%", border: "none", borderRadius: 12, padding: "15px 18px", cursor: "pointer", background: C.blue, color: "#fff", fontFamily: C.sans, fontSize: 15, fontWeight: 750 }}>
              Sigue el próximo
            </button>
          </div>
        </div>
        ...
        <div role="dialog" aria-modal="true" aria-label="Código sin cargar"
          ...
          <div style={{ ...panel, width: "100%", maxWidth: 560, maxHeight: "min(88vh, 720px)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "16px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "flex-start", gap: 11 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ color: C.text, fontSize: 15, fontWeight: 750 }}>Este código no está cargado</div>
                <div style={{ color: C.dim, fontSize: 12, fontWeight: 650, marginTop: 2, fontFamily: C.mono }}>{desconocido}</div>
                <div style={{ color: C.dim, fontSize: 12, fontWeight: 600, marginTop: 6 }}>Elegí de qué producto es y queda vinculado para siempre.</div>
              </div>
            ...
            <div style={{ flex: 1, minHeight: 0, overflowY: "auto" }}>
              {!candidatos.length ? (
                <div style={{ padding: "28px 18px", textAlign: "center", color: C.dim, fontSize: 12.5, fontWeight: 650 }}>
                ...
                <button key={m.id} type="button" className="caja-item" onClick={() => asignarCodigo(m)}
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ color: C.text, fontSize: 13.5, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.descripcion}</div>
                    <div style={{ color: C.dim, fontSize: 11, fontWeight: 600 }}>{m.rubro} · {m.unidad}</div>
                  </div>
                  <span style={{ fontFamily: C.mono, fontSize: 12, fontWeight: 700, color: m.stock > 0 ? C.text : C.dim, flexShrink: 0 }}>{fmt(m.stock)}</span>
                </button>
              ))}
            </div>
          </div>
