...
function StockGeneralModal({ open, onClose, onDone, sedeLocked, isMobile, toast, egresoRapido = false, obras = [] }) {
  ...
  // Búsqueda server-side. Con búsqueda vacía NO se baja el catálogo entero: eso era la
  // espera de ~30s al abrir. Sin término, el listado queda vacío hasta que se escribe.
  useEffect(() => {
    if (!open) return undefined;
    const term = q.trim();
    if (!term) { setCatalog([]); setLoading(false); return undefined; }
    let alive = true;
    setLoading(true);
    const timer = setTimeout(async () => {
      try {
        const rows = await fetchPanolCatalogMini({ q: term, limit: 80 });
            ...
            <div style={{ border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", background: C.panel }}>
              <div style={{ maxHeight: 280, overflowY: "auto", display: "grid", gap: 0 }}>
                {loading ? (
                  ...
                  <div style={{ color: C.dim, padding: 24, textAlign: "center", fontSize: 13 }}>{q.trim() ? "Sin resultados" : "Escribí para buscar un material"}</div>
                )}
              </div>
            </div>
