...
export function UbicacionChip({ ubicacion, obs = null, size = "sm" }) {
  ...
  if (!cod) return null;
  const color = afuera ? null : zonaColor(cod);
  const label = afuera ? "Afuera del pañol" : nivel ? `${cod} · ${nivel}º` : cod;
  return (
    <span title={obs ? `${label} — ${obs}` : label} style={{
      display: "inline-flex", alignItems: "center", gap: 4,
      fontSize: size === "sm" ? 10 : 11.5, fontWeight: 600, fontFamily: C.mono,
      color: afuera ? "var(--cyan)" : color,
      border: afuera ? "1px solid var(--cyan-border)" : `1px solid ${color}55`,
      background: afuera ? "var(--cyan-soft)" : `${color}14`,
      borderRadius: 6, padding: size === "sm" ? "1px 6px" : "3px 8px", whiteSpace: "nowrap",
    }}>
...
export default function UbicacionPicker({ materialId, ubicacion = null, ubicacionObs = null, onSaved, toast, label = "Ubicación en el pañol" }) {
  ...
  return (
    <div style={{ display: "grid", gap: 6 }}>
      <span style={{ color: C.dim, fontSize: 11, fontWeight: 600, letterSpacing: ".07em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {dirty && (
          <button type="button" onClick={guardar} disabled={saving || !materialId} className="ui-btn ui-btn-suave">
