      ...
      {modo === "tabla" && (
        <div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ position: "sticky", top: 0, background: C.panelSolid, zIndex: 1 }}>
                <tr>
                  ...
                  <th style={{ padding: "8px 12px", textAlign: "left",  fontSize: 10, color: C.t2, letterSpacing: 1.3, textTransform: "uppercase", borderBottom: `1px solid ${C.b0}`, width: 90 }}>Categoría</th>
                  ...
                  <th style={{ padding: "8px 12px", textAlign: "right", fontSize: 10, color: C.t2, letterSpacing: 1.3, textTransform: "uppercase", borderBottom: `1px solid ${C.b0}`, width: 130 }}>Real contado</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={mat.id} style={{ background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.012)" }}>
                      <td style={{ padding: "7px 12px", borderBottom: `1px solid var(--panel)`, fontSize: 13, color: C.t0 }}>
                        {mat.nombre}
                        {mat.unidad_medida && <span style={{ fontSize: 10, color: C.t2, marginLeft: 5 }}>{mat.unidad_medida}</span>}
                      </td>
                      <td style={{ padding: "7px 12px", borderBottom: `1px solid var(--panel)`, fontSize: 11, color: C.t2 }}>
                        {mat.categoria || "—"}
                      </td>
                      ...
                      <td style={{ padding: "5px 12px", borderBottom: `1px solid var(--panel)` }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
                          {diff !== null && !isNaN(diff) && diff !== 0 && (
                            <span style={{ fontSize: 12, fontFamily: C.mono, fontWeight: 600, color: diff > 0 ? C.green : C.red }}>
                              {diff > 0 ? `+${diff}` : diff}
                            </span>
                          )}
                          <input
          ...
          <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end" }}>
            <button onClick={confirmarTabla} style={{ border: "1px solid rgba(59,130,246,0.35)", background: "rgba(59,130,246,0.15)", color: "#60a5fa", padding: "8px 20px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: C.sans }}>
              Ver diferencias →
            </button>
          </div>
        </div>
      ...
      {modo === "texto" && (
        <div>
          ...
          <div style={{ marginTop: 10, display: "flex", justifyContent: "flex-end" }}>
            <button onClick={() => procesar(parsearTexto(texto))} style={{ border: "1px solid rgba(59,130,246,0.35)", background: "rgba(59,130,246,0.15)", color: "#60a5fa", padding: "8px 20px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: C.sans }}>
              Ver diferencias →
            </button>
          </div>
        </div>
      ...
      {modo === "archivo" && (
        <div>
          <div style={{ fontSize: 12, color: C.t2, marginBottom: 12, lineHeight: 1.6 }}>
            ...
            Desde Excel: <span style={{ fontFamily: C.mono, fontSize: 12, color: C.t1 }}>Archivo → Guardar como → CSV UTF-8</span>
          </div>
          <div
          ...
          >
            ...
            <div style={{ fontSize: 13, color: C.t1 }}>Hacer click para seleccionar</div>
...
function PasoPreview({ lineas, observaciones, setObservaciones, onConfirmar, onVolver, saving }) {
  ...
  if (!diffs.length) return (
    <div style={{ textAlign: "center", padding: "40px 20px" }}>
      ...
      <div style={{ fontSize: 13, color: C.t2 }}>Los {lineas.length} materiales coinciden con el stock del sistema.</div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <div style={{ flex: 1, background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 8, padding: "10px 14px" }}>
          ...
          <div style={{ fontFamily: C.mono, fontSize: 22, fontWeight: 600, color: C.green }}>{positivos}</div>
        </div>
        <div style={{ flex: 1, background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "10px 14px" }}>
          ...
          <div style={{ fontFamily: C.mono, fontSize: 22, fontWeight: 600, color: C.red }}>{negativos}</div>
        </div>
        {sinCambio > 0 && (
          <div style={{ flex: 1, background: "var(--panel)", border: `1px solid ${C.b0}`, borderRadius: 8, padding: "10px 14px" }}>
            ...
            <div style={{ fontFamily: C.mono, fontSize: 22, fontWeight: 600, color: C.t2 }}>{sinCambio}</div>
          </div>
        )}
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead style={{ position: "sticky", top: 0, background: C.panelSolid }}>
            <tr>
              ...
              <th style={{ padding: "8px 12px", textAlign: "right", fontSize: 10, color: C.t2, letterSpacing: 1.3, textTransform: "uppercase", borderBottom: `1px solid ${C.b0}` }}>Ajuste</th>
            </tr>
          </thead>
          <tbody>
            {diffs.map(({ material, stockActual, realQty, diff }) => (
              <tr key={material.id} style={{ background: diff > 0 ? "rgba(16,185,129,0.025)" : "rgba(239,68,68,0.025)" }}>
                <td style={{ padding: "8px 12px", borderBottom: `1px solid var(--panel)`, fontSize: 13, color: C.t0 }}>
                  {material.nombre}
                  {material.unidad_medida && <span style={{ fontSize: 10, color: C.t2, marginLeft: 5 }}>{material.unidad_medida}</span>}
                </td>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <button onClick={onVolver} disabled={saving} style={{ border: `1px solid ${C.b0}`, background: "transparent", color: C.t1, padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontFamily: C.sans }}>
          ← Volver
        </button>
        <button onClick={onConfirmar} disabled={saving} style={{ border: "1px solid rgba(16,185,129,0.35)", background: "rgba(16,185,129,0.15)", color: "#34d399", padding: "8px 22px", borderRadius: 8, cursor: saving ? "not-allowed" : "pointer", fontWeight: 600, fontSize: 13, fontFamily: C.sans, opacity: saving ? 0.5 : 1 }}>
...
export default function AjusteMaderasModal({ materiales, onClose, onDone }) {
  ...
  return (
    <div
      ...
      style={{ position: "fixed", inset: 0, zIndex: 9500, background: "var(--overlay-strong)", ...GLASS, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "32px 20px", overflowY: "auto" }}
    >
      <div style={{ background: C.panelSolid, border: `1px solid ${C.b1}`, borderRadius: 14, width: "100%", maxWidth: 680, boxShadow: "0 32px 80px rgba(0,0,0,0.8)", fontFamily: C.sans, marginBottom: 40 }}>
        ...
        <div style={{ padding: "18px 22px 14px", borderBottom: `1px solid ${C.b0}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 17, fontWeight: 600, color: C.t0 }}>Ajuste de inventario · Maderas</div>
        ...
        <div style={{ padding: "20px 22px 24px" }}>
          ...
          {saving && progreso.total > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12, color: C.t2 }}>
