...
function tonoDeCobertura(cobertura) {
  if (cobertura >= 0.75) return { color: C.green, peso: 700 };
  if (cobertura >= 0.4) return { color: C.text, peso: 650 };
  return { color: C.dim, peso: 600 };
}
...
export default function MatrizLineasPanel({ isMobile = false }) {
  ...
  const th = {
    ...
    textAlign: "left", fontSize: 10.5, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase",
    ...
    <div style={{ display: "grid", gap: 14, padding: isMobile ? 12 : 0, minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ color: C.text, fontSize: 16, fontWeight: 750 }}>Matriz por línea</div>
        ...
        <select
          ...
          style={{ border: `1px solid ${C.border2}`, background: C.panelSolid, color: C.text, borderRadius: 9, padding: "8px 10px", fontFamily: C.sans, fontSize: 12.5, fontWeight: 650, outline: "none" }}
        >
        ...
        <button
          ...
          style={{ border: `1px solid ${soloFaltantes ? C.blueB : C.border2}`, background: soloFaltantes ? C.blueL : C.panelSolid, color: soloFaltantes ? C.blue : C.text, borderRadius: 9, padding: "8px 11px", cursor: "pointer", fontFamily: C.sans, fontSize: 12.5, fontWeight: 700 }}
        >
      ...
      {datos ? (
        <div style={{ ...tarjeta, padding: "11px 14px", display: "flex", gap: 18, flexWrap: "wrap", alignItems: "center" }}>
          <div>
            <span style={{ color: C.text, fontSize: 19, fontWeight: 750, fontFamily: C.mono }}>{datos.resumen.materiales}</span>
            <span style={{ color: C.muted, fontSize: 12.5, fontWeight: 650, marginLeft: 7 }}>materiales en la matriz</span>
          </div>
          <div>
            <span style={{ color: C.blue, fontSize: 19, fontWeight: 750, fontFamily: C.mono }}>{datos.resumen.compartidos}</span>
            <span style={{ color: C.muted, fontSize: 12.5, fontWeight: 650, marginLeft: 7 }}>van a más de una línea</span>
          </div>
      ...
      <div style={{ ...tarjeta, background: C.cyanL, borderColor: C.cyanB, padding: "10px 14px", fontSize: 12, color: C.muted, fontWeight: 650, lineHeight: 1.5 }}>
      ...
      {cargando && !datos ? (
        ...
        <div style={{ ...tarjeta, overflowX: "auto" }}>
                ...
                {columnas.map((l) => (
                  <th key={l.nombre} style={{ ...th, textAlign: "center", minWidth: 78 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                      ...
                      <span style={{ fontSize: 9.5, color: C.dim, fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>
                        {l.activas} en curso
                      </span>
                    </div>
                  </th>
              ...
              {filas.slice(0, 300).map((f, i) => (
                <tr key={f.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${C.border}` }}>
                  <td style={{ padding: "9px 10px", minWidth: 0 }}>
                    <div style={{ color: C.text, fontSize: 12.5, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 260 }}>
                      {f.descripcion}
                    </div>
                    <div style={{ color: C.dim, fontSize: 10.5, fontWeight: 600 }}>
                      {f.unidad}
                      {f.lineasQueLoUsan > 1 ? ` · ${f.lineasQueLoUsan} líneas` : ""}
                      {f.esConsumible ? " · consumible" : ""}
                    </div>
                  </td>
                  <td style={{ padding: "9px 10px", color: f.proveedor ? C.muted : C.red, fontSize: 11.5, fontWeight: 650, whiteSpace: "nowrap" }}>
                  ...
                  {columnas.map((l) => {
                    ...
                    return (
                      <td key={l.nombre} style={{ padding: "9px 6px", textAlign: "center" }}>
                        ...
                        <div style={{ color: C.dim, fontSize: 9.5, fontWeight: 600 }}>
                          {celda.obrasQueLoUsaron}/{celda.obrasDeLaLinea}
                        </div>
                      </td>
                    );
                  })}
                  <td style={{ padding: "9px 10px", textAlign: "right", color: C.muted, fontSize: 12.5, fontWeight: 650, fontFamily: C.mono }}>
                    {f.hay}
                  </td>
                  <td style={{ padding: "9px 10px", textAlign: "right", fontFamily: C.mono, fontSize: 12.5, fontWeight: 700, color: f.faltan > 0 ? C.red : C.dim }}>
                    {f.faltan > 0 ? f.faltan : "—"}
                  </td>
                </tr>
              ))}
          ...
          {filas.length > 300 ? (
            <div style={{ padding: "10px 14px", color: C.dim, fontSize: 11.5, fontWeight: 600, borderTop: `1px solid ${C.border}` }}>
