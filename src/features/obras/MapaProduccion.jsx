...
export default function MapaProduccion({obras=[],onPuestoClick,onAsignarObra,onChangeEstado,esGestion=false,sharedPuestos,onSaveLayout,sharedNotas,onSaveNotas,sharedMemorias,onSaveMemorias,onAsignarObraPampa,onDesasignarObraPampa}){
  ...
  useLayoutEffect(()=>{
    const fit=()=>{
      ...
      // con el listener de resize (set-state-in-effect) y parpadeos.
      if(Math.abs(cur.x-next.x)<0.5&&Math.abs(cur.y-next.y)<0.5&&Math.abs(cur.scale-next.scale)<0.001) return;
  ...
  const renderBoat=(p)=>{
    ...
    return(
      ...
      <g transform={sc!==1?`translate(${p.cx*(1-sc)},${p.cy*(1-sc)}) scale(${sc})`:undefined} style={{transition:"transform 0.2s cubic-bezier(0.175,0.885,0.32,1.275)"}}>
          ...
          {/* Sonar ping para pausadas — violeta, coherente con C.obra.pausada */}
          {isPaused&&(
            <g style={{pointerEvents:"none", userSelect:"none"}}>
              <circle cx={p.cx} cy={p.cy} r="0" fill="none" stroke="var(--violet)" strokeWidth="1.5" style={{animation:"sonarPing 2.8s ease-out infinite"}}/>
          ...
          {/* ── EDIT MODE OVERLAY ── */}
          {editMode&&(<>
            <rect x={ix} y={iy} width={p.w} height={p.h} rx={Math.min(p.w,p.h)*0.07} fill="rgba(96,165,250,0.06)" stroke="rgba(96,165,250,0.38)" strokeWidth="1.3" strokeDasharray="4 3" style={{pointerEvents:"none"}}/>
            ...
            {hasFicha&&(
              <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:10,paddingBottom:10,borderBottom:"1px solid var(--panel-2)"}}>
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:3}}>
                  {obra.grupo_electrogeno&&<span style={{fontSize:10,padding:"2px 7px",borderRadius:4,background:"rgba(167,139,250,0.12)",color:"#c4b5fd",border:"1px solid rgba(167,139,250,0.22)"}}>{obra.grupo_electrogeno_det||"Grupo elect."}</span>}
      ...
      {confirmDel&&(
        <div style={{position:"fixed",inset:0,zIndex:1200,background:"var(--overlay-strong)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}} onClick={()=>setConfirmDel(null)}>
          <div style={{...GLASS,border:"1px solid rgba(239,68,68,0.4)",borderRadius:16,padding:"24px",width:320,animation:"fadeUp 0.15s ease",boxShadow:"0 24px 48px rgba(0,0,0,0.9),0 0 0 1px rgba(239,68,68,0.2) inset"}} onClick={e=>e.stopPropagation()}>
            ...
            {obraByPuesto[confirmDel]&&<div style={{fontSize:13,color:"#67e8f9",marginBottom:24,padding:"10px 12px",borderRadius:8,background:"rgba(103,232,249,0.08)",border:"1px solid rgba(103,232,249,0.3)"}}>La obra asignada quedará sin puesto en el plano.</div>}
