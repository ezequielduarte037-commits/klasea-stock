        ...
        <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ fontSize: 10, color: C.t2, letterSpacing: 1 }}>Producción</div>
              {lineaSel && (
                <>
                  <div style={{ width: 1, height: 14, background: C.b0 }} />
                  <div style={{ fontSize: 11, color: C.t1 }}>{lineaSel.nombre}</div>
                  {unidadSel && (
                    <>
                      <div style={{ fontSize: 10, color: C.t2 }}>›</div>
                      <div style={{ fontSize: 11, color: C.t0, fontWeight: 600 }}>{unidadSel.codigo}</div>
                    </>
                  )}
                </>
              )}
            ...
            {unidadId && (
              <div style={{ display: "flex", gap: 7 }}>
                {[
                  { label: "Completos", n: stats.completo, color: C.green  },
                  { label: "Pendientes", n: stats.total - stats.completo - stats.parcial - stats.rehacer, color: C.t2 },
                  ...(stats.rehacer > 0 ? [{ label: "Rehacer", n: stats.rehacer, color: C.red }] : []),
                ].map(({ label, n, color }) => (
                  <div key={label} style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: 6, background: C.s0, border: `1px solid ${C.b0}` }}>
                    <span style={{ fontFamily: C.mono, fontSize: 13, fontWeight: 700, color, lineHeight: 1 }}>{n}</span>
                    <span style={{ fontSize: 8, color: C.t1, letterSpacing: 2, textTransform: "uppercase" }}>{label}</span>
                  </div>
                ))}
                <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: 6, background: pctColor === C.green ? "rgba(16,185,129,0.1)" : C.s0, border: `1px solid ${pctColor === C.green ? "rgba(16,185,129,0.25)" : C.b0}` }}>
                  <span style={{ fontFamily: C.mono, fontSize: 13, fontWeight: 700, color: pctColor, lineHeight: 1 }}>{pct}%</span>
                </div>
              </div>
            )}
          </div>
          ...
          {/* ── LEFT NAV ── */}
          <div style={{ height: "100%", overflowY: "auto", borderRight: `1px solid ${C.b0}`, background: "rgba(9,9,11,0.98)", display: "flex", flexDirection: "column" }}>
            {/* Header */}
