-- Remitos de recepcion: cada item puede ingresar directo al stock de panol de una obra.

alter table public.panol_envios
  drop constraint if exists panol_envios_origen_check;

alter table public.panol_envios
  add constraint panol_envios_origen_check
  check (origen in ('compra','manual','remito'));
...
begin
  ...
  for v_it in select * from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) loop
    ...
    if v_snapshot is null and v_obra_item is not null then
      ...
      values (
        ...
        nullif(v_it->>'rubro', ''),
        coalesce(nullif(v_it->>'tipo', ''), case when p_origen = 'remito' then 'remito' else 'manual' end),
        coalesce(nullif(v_it->>'tipo_label', ''), case when p_origen = 'remito' then 'Remito' else 'Manual' end),
        v_precio,
